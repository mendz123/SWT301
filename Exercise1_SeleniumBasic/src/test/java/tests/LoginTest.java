...
public class LoginTest {
    ...
    @ParameterizedTest(name = "Validate input: {0} / {1}")
    @CsvSource({
            "'', SuperSecretPassword!, error",               // empty username
            "tomsmith, '', error",                           // empty password
            "invalidEmail, SuperSecretPassword!, error",     // invalid email format
            "123456789, SuperSecretPassword!, error",        // phone number as username
            "tomsmith, short, error"                         // short password
    })
    @Order(5)
    @DisplayName("Should validate various input combinations")
    void testLoginInputValidation(String username, String password, String expected) {
        loginPage.navigate();
        username = (username == null) ? "" : username.trim();
        password = (password == null) ? "" : password.trim();
